...
def new_fed(update, context):
        ...
        if chat.type != "private":
                send_message(update.effective_message, "Create your federation in my PM, not in the group.")
                return
        ...
        if not fednam == '':
                ...
                if not x:
                        send_message(update.effective_message, "Can't create fed! Please contact my creator if the problem still persists.")
                        return

                send_message(update.effective_message, "*You have created a new federation!*\nName: `{}`\nID: `{}`\n\nUse the command below to join the federation:\n`/joinfed {}`".format(fed_name, fed_id, fed_id), parse_mode=ParseMode.MARKDOWN)
                try:
                        context.bot.send_message(TEMPORARY_DATA,
                                "Federation <b>{}</b> has been created with an ID: <pre>{}</pre>".format(fed_name, fed_id), parse_mode=ParseMode.HTML)
                except:
...
def del_fed(update, context):
        ...
        args = context.args
        if chat.type != "private":
                send_message(update.effective_message, "Delete your federation in my PM, not in the group.")
                return
        if args:
                is_fed_id = args[0]
                getinfo = sql.get_fed_info(is_fed_id)
                if getinfo == False:
                        send_message(update.effective_message, "This federation is not found!")
                        return
        ...
        send_message(update.effective_message, "Are you sure you want to delete your federation? This action cannot be undone - you will lose your entire ban list, and '{}' will be permanently gone.".format(getinfo['fname']),
...
def fed_chat(update, context):
        ...
        if not is_user_admin(update.effective_chat, user_id):
                send_message(update.effective_message, "You must be an admin to execute this command")
                return
...
def join_fed(update, context):
                ...
                for admin in administrators:
                        ...
                        if status == "creator":
                                if str(admin.user.id) == str(user.id):
                                        ...
                                        send_message(update.effective_message, "Only group creators can do it!")
                                        return
        if fed_id:
                send_message(update.effective_message, "You cannot join two federations in one chat")
                return
        ...
        if len(args) >= 1:
                ...
                if not x:
                        send_message(update.effective_message, "Failed to join federation! Please contact my creator if the problem still persists.")
                        return
...
def leave_fed(update, context):
        ...
        if getuser in 'creator' or user.id in SUDO_USERS:
                if sql.chat_leave_fed(chat.id) == True:
                        ...
                        send_message(update.effective_message, "Why did you leave the federation when you haven't joined?!")
        else:
                send_message(update.effective_message, "Only group creators can do it!")
...
def user_join_fed(update, context):
        ...
        if is_user_fed_owner(fed_id, user.id):
                ...
                if user_id and user_id != "error":
                        ...
                        send_message(update.effective_message, "I can't extract this user.")
                        return
                else:
                ...
                if user_id == get_owner:
                        send_message(update.effective_message, "Why are you trying to promote federated owner!?")
                        return
                if getuser:
                        send_message(update.effective_message, "I cannot promote users who are already federated admins! But I can demote him.")
                        return
...
def user_demote_fed(update, context):
        ...
        if is_user_fed_owner(fed_id, user.id):
                ...
                if sql.search_user_in_fed(fed_id, user_id) == False:
                        send_message(update.effective_message, "I cannot demote users who are not federated admins! If you want to make him cry, Promote him first!")
                        return
                ...
                admin = FEDADMIN
                if res:
                        send_message(update.effective_message, f"{admin} demoted {mention_html(user.id, user.first_name)} in {fed_info['fname']}")
                else:
                        send_message(update.effective_message, "Failed to promote!")
                if FED_LOGS:
                        context.bot.send_message(FED_LOGS,
...
def fed_admin(update, context):
        ...
        if is_user_fed_admin(fed_id, user.id) == False:
                send_message(update.effective_message, "Only federated admin can do this!")
                return
...
def fed_ban(update, context):
        ...
        if not user_id:
                send_message(update.effective_message, "You do not seem to be referring to the user.")
                return

        if user_id == context.bot.id:
                send_message(update.effective_message, "What's funnier than kicking the group creator? Fban myself.")
                return

        if is_user_fed_owner(fed_id, user_id) == True:
                send_message(update.effective_message, "Why did you try fban owner federation?")
                return

        if is_user_fed_admin(fed_id, user_id) == True:
                send_message(update.effective_message, "He is a federation admin, I can't fban him.")
                return

        if user_id == OWNER_ID:
                send_message(update.effective_message, "I don't want to fban my master, that's a very stupid idea!")
                return

        if int(user_id) in SUDO_USERS:
                send_message(update.effective_message, "I will not fban users sudo!")
                return

        if int(user_id) in WHITELIST_USERS:
                send_message(update.effective_message, "This person is whitelisted, so i can't fban him")
                return
        ...
        if fban:
                fed_name = info['fname']
                starting = "The reason for fban is changed for {} in the Federation <b>{}</b>.".format(user_target, fed_name)
                ...
                if not temp:
                        send_message(update.effective_message, "Failed to update the reason for fedban!")
                        return
                x = sql.fban_user(fed_id, fban_user_id, fban_user_name, fban_user_lname, fban_user_uname, reason, int(time.time()))
                if not x:
                        send_message(update.effective_message, "Failed to fbanning user! If this problem continues, contact my creator.")
                        return
                ...
                if get_fedlog:
                        if int(get_fedlog) != int(chat.id):
                                context.bot.send_message(get_fedlog, "<b>FedBan Reason is updated</b>\n<b>Fed:</b> {}\n<b>Fed Admin:</b> {}\n<b>User:</b> {}\n<b>User ID:</b> <code>{}</code>\n<b>Reason:</b> {}".format(fed_name, mention_html(user.id, user.first_name), user_target, fban_user_id, reason), parse_mode="HTML")
                ...
                for fedsid in subscriber:
                        ...
                        for fedschat in all_fedschat:
                                try:
                                        ...
                                        pass
        send_message(update.effective_message, "This person has been fbanned.")
...
def unfban(update, context):
        ...
        if fban == False:
                send_message(update.effective_message, "This user is not fbanned!")
                return

        banner = update.effective_user  # type: Optional[User]

        send_message(update.effective_message, "I will give {} second chance in this federation.".format(user_target), parse_mode="HTML")
        ...
        try:
                ...
                if not x:
                        send_message(update.effective_message, "Fban failed, this user may already un-fedbanned!")
                        return
        except:
                pass
...
def set_frules(update, context):
        ...
        if len(args) >= 1:
                ...
                send_message(update.effective_message, "Rules have been changed to:\n{}!".format(rules))
        else:
                send_message(update.effective_message, "Please write rules to set it up")
...
def fed_ban_list(update, context):
        ...
        getfban = sql.get_all_fban_users(fed_id)
        if len(getfban) == 0:
                send_message(update.effective_message, "There are no fbanned users on the federation {}".format(info['fname']), parse_mode=ParseMode.HTML)
                return
                ...
                if args[0] == 'json':
                        ...
                        if cek.get('status'):
                                if jam <= int(cek.get('value')):
                                        ...
                                        send_message(update.effective_message, "You can get data every 30 minutes!\nYou can get this data again at `{} WIB`".format(waktu), parse_mode=ParseMode.MARKDOWN)
                                        return
                                else:
                        ...
                        with BytesIO(str.encode(backups)) as output:
                                ...
                                update.effective_message.reply_document(document=output, filename="emilia_fbanned_users.json",
                                                                                                        caption="Total {} user is fbanned by federation {}.".format(len(getfban), info['fname']))
                        return
                elif args[0] == 'csv':
                        jam = time.time()
                        new_jam = jam + 1800
        ...
        text = "<b>{} users who are fbanned in federation {}:</b>\n".format(len(getfban), info['fname'])
        for users in getfban:
                getuserinfo = sql.get_all_fban_users_target(fed_id, users)
                if getuserinfo == False:
                        text = "There are no fbanned users on the federation {}".format(info['fname'])
                        break
        ...
        try:
                ...
                with BytesIO(str.encode(cleantext)) as output:
                        ...
                        update.effective_message.reply_document(document=output, filename="fbanlist.txt",
                                                                                                        caption="The following is a list of users who are currently fbanned on the federation {}.".format(info['fname']))
...
def fed_notif(update, context):
        ...
        if args:
                if args[0] in ("yes", "on", "ya"):
                        ...
                        send_message(update.effective_message, "I will notify you if there any users get fban/unfban in PM.")
                elif args[0] in ("no", "off", "ga"):
                        sql.set_feds_setting(user.id, False)
                        send_message(update.effective_message, "I will not notify you if there any users get fban/unfban in PM.")
                else:
                        send_message(update.effective_message, "Please enter `on`/`off`", parse_mode="markdown")
        else:
                getreport = sql.user_feds_report(user.id)
                send_message(update.effective_message, "Your current federation report preferences: `{}`".format(getreport), parse_mode="markdown")
...
def fed_chats(update, context):
        ...
        if len(getlist) == 0:
                send_message(update.effective_message, "There is no chat joining the federation {}".format(info['fname']), parse_mode=ParseMode.HTML)
                return

        text = "<b>Chat that joined the federation {}:</b>\n".format(info['fname'])
        ...
        try:
                ...
                with BytesIO(str.encode(cleantext)) as output:
                        ...
                        update.effective_message.reply_document(document=output, filename="fedchats.txt", caption="The following is a list of chats that joined the federation {}.".format(info['fname']))
...
def del_fed_button(update, context):
        ...
        if fed_id == 'cancel':
                query.message.edit_text("Delete federation was canceled")
                return
        ...
        if getfed:
                ...
                if delete:
                        query.message.edit_text("You have deleted your federation! Now all the Groups are connected with `{}` has no federation.".format(getfed['fname']), parse_mode='markdown')
...
def fed_import_bans(update, context):
        ...
        if msg.reply_to_message and msg.reply_to_message.document:
                ...
                try:
                        ...
                        send_message(update.effective_message, "Try downloading and re-uploading the file, this one seems broken!")
                        return
...
def __stats__():
        ...
        return "{} users has fbanned, on {} federation".format(len(all_fbanned), len(all_feds))
...
def __user_info__(user_id, chat_id):
        ...
        if fed_id:
                ...
                if int(info['owner']) == user_id:
                        text = "This user is owner of the federation: <b>{}</b>.".format(infoname)
                elif is_user_fed_admin(fed_id, user_id):
                        text = "This user is admin of the federation: <b>{}</b>.".format(infoname)
